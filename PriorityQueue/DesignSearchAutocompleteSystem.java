...
class AutocompleteSystem {
    ...
    public void insert(String str, int time){
        ...
        for(int i = 0; i < str.length(); i++){
            if(node.children[(int)str.charAt(i)] == null){
                node.children[(int)str.charAt(i)] = new TrieNode(str.charAt(i));
                node = node.children[(int)str.charAt(i)];
            }
            else
                node = node.children[(int)str.charAt(i)]; 
        }
    ...
    public List<String> input(char c) {
        ...
        int count = 3;
        while(count > 0){
            if(!pq.isEmpty())res.add(pq.poll().sentence);
            count--;
        }
        return res;
    ...
    public void dfs(TrieNode root,PriorityQueue<TrieNode> pq){
        if(root == null) return;
        TrieNode node = root;
        if(node.isSentence) pq.add(node);
